...
public class RGHSettings extends ScriptableSystem {
    ...
    @runtimeProperty("ModSettings.mod", "Ragdoll Gravity Helper")
    ...
    public let forceStrength: Float = 6.0;

    @runtimeProperty("ModSettings.mod", "Ragdoll Gravity Helper")
    @runtimeProperty("ModSettings.displayName", "Max Assist Duration")
    @runtimeProperty("ModSettings.description", "Safety cap (seconds): stop assisting a ragdoll after this long even if it never settles.")
    @runtimeProperty("ModSettings.step", "0.5")
    @runtimeProperty("ModSettings.min", "1.0")
    @runtimeProperty("ModSettings.max", "15.0")
    @runtimeProperty("ModSettings.dependency", "enabled")
    public let maxDurationSec: Float = 3.0;
}
